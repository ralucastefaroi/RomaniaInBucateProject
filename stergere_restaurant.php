...
if(isset($_POST['id_restaurant']) && !empty($_POST["id_restaurant"])){
    require_once "config.php";
    $sql = "DELETE FROM restaurante WHERE id_restaurant = ? LIMIT 1";
    
    if($stmt = mysqli_prepare($mysqli, $sql)){
        mysqli_stmt_bind_param($stmt, "i", $param_id_restaurant);
        
        
        $param_id_restaurant = trim($_POST["id_restaurant"]);
...
<body>
    ...
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="mt-5 mb-3">Ștergere restaurant</h2>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div>
                            <input type="hidden" name="id_restaurant" value="<?php echo trim($_GET["id"]); ?>"/>
                            <p class="color">Sigur ștergeți acest restaurant?</p>
                                <input type="submit" value="Da" class="btn" style="background-color: #e6ccb3;color:#663300">
                                <a href="vizualizare_restaurante.php" class="btn" style="background-color: #e6ccb3;color:#663300">Nu</a>
                            </p>
                        </div>
                    </form>
                </div>
