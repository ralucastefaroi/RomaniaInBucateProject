...
if(isset($_POST['id_reteta']) && !empty($_POST["id_reteta"])){
    require_once "config.php";

    $sql = "DELETE FROM retete WHERE id_reteta = ? LIMIT 1";
    
    if($stmt = mysqli_prepare($mysqli, $sql)){

        mysqli_stmt_bind_param($stmt, "i", $param_id_reteta);
        
        $param_id_reteta = trim($_POST["id_reteta"]);
...
<body>
    ...
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="mt-5 mb-3">Ștergere rețetă</h2>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div>
                            <input type="hidden" name="id_reteta" value="<?php echo trim($_GET["id"]); ?>"/>
                            <p class="color">Sigur ștergeți această rețetă?</p>
                                <input type="submit" value="Da" class="btn" style="background-color: #e6ccb3;color:#663300">
                                <a href="vizualizare_retete.php" class="btn" style="background-color: #e6ccb3;color:#663300">Nu</a>
                            </p>
                        </div>
                    </form>
                </div>
